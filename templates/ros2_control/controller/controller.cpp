...
{
  auto if_empty_return_error = [&](const auto & parameter, const char * parameter_name) {
    if (parameter.empty()) {
  ...
  auto get_string_param_and_if_empty_return_error = [&](std::string & parameter,
                                                        const char * parameter_name) {
    parameter = get_node()->get_parameter(parameter_name).as_string();
    return if_empty_return_error(parameter, parameter_name);
  };

  joint_names_ = get_node()->get_parameter("joints").as_string_array();

  if (if_empty_return_error(joint_names_, "joints") ||
      get_string_param_and_if_empty_return_error(interface_name_, "interface_name"))
  {
  ...
  state_publisher_ = std::make_unique<StatePublisher>(s_publisher_);
