...
//     std::make_tuple(std::string("state_joints"), rclcpp::ParameterValue(std::vector<std::string>({}))),
...
TEST_F(DummyClassNameTest, test_setting_slow_mode_service)
{
  ...
  ASSERT_FALSE(controller_->slow_control_mode_.readFromRT());
  ...
  ASSERT_TRUE(controller_->slow_control_mode_.readFromRT());
...
//
//   ASSERT_FALSE(controller_->slow_control_mode_.readFromRT());
//   ASSERT_EQ(joint_command_values_[STATE_MY_ITFS], TEST_DISPLACEMENT);
//   ASSERT_TRUE(std::isnan((*(controller_->input_cmd_.readFromRT()))->displacements[0]));
//   ASSERT_FALSE(controller_->slow_control_mode_.readFromRT());
//
//   // When slow mode is enabled command ends up being half of the value
//   controller_->slow_control_mode_.writeFromNonRT(true);
//   msg->displacements.resize(joint_names_.size(), TEST_DISPLACEMENT);
//   controller_->input_cmd_.writeFromNonRT(msg);
//   ASSERT_EQ(controller_->update(rclcpp::Time(0), rclcpp::Duration::from_seconds(0.01)), controller_interface::return_type::OK);
//
//   ASSERT_TRUE(controller_->slow_control_mode_.readFromRT());
//   ASSERT_EQ(joint_command_values_[STATE_MY_ITFS], TEST_DISPLACEMENT/2);
//   ASSERT_TRUE(std::isnan((*(controller_->input_cmd_.readFromRT()))->displacements[0]));
// }
//
