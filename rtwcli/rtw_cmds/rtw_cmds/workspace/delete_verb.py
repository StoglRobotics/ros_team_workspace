...
class RemovalStep(ABC):
    ...
    def execute(self, workspace: Workspace, config: WorkspacesConfig) -> StepStats:
        if self.should_run(workspace):
            logger.info(f"Starting {self.name.value}")
            try:
                if self.run(workspace, config):
                    self.status = StepStatus.COMPLETED
                    self.message = f"Successfully completed {self.name.value}"
                    logger.info(self.message)
                else:
                    self.status = StepStatus.FAILED
                    self.message = f"Failed to complete {self.name.value}"
                    logger.error(self.message)
            except Exception as e:
                self.status = StepStatus.FAILED
                self.message = f"Error in {self.name.value}: {str(e)}"
                logger.error(self.message)
                raise
        else:
            self.status = StepStatus.SKIPPED
            self.message = f"Skipping {self.name.value} (not applicable)"
            logger.info(self.message)
...
class RemoveWorkspaceFolder(RemovalStep):
    ...
    def should_run(self, workspace: Workspace) -> bool:
        return not workspace.standalone and os.path.exists(workspace.ws_folder)
    ...
    def run(self, workspace: Workspace, config: WorkspacesConfig) -> bool:
        ...
        for item in items_to_delete:
            item_path = os.path.join(workspace.ws_folder, item)
        ...
        self.message = (
            f"Removed items {items_to_delete} from workspace folder '{workspace.ws_folder}'"
        )
        return True
...
def remove_workspaces(
    ...
    for ws_name in workspace_names:
        ...
        if not workspace:
            removal_stats[ws_name] = WorkspaceRemovalStats(
                status=WorkspaceRemovalStatus.FAILED,
                message=f"Workspace '{ws_name}' not found in the configuration",
            )
...
def print_removal_stats(stats: Dict[str, WorkspaceRemovalStats]):
    tree = Tree("Workspace Removal Statistics")
    for ws_name, ws_stats in stats.items():
        ws_node = tree.add(f"{ws_name} - {ws_stats.status.name}")
        ws_node.add(ws_stats.message)
        for step in ws_stats.steps:
            step_node = ws_node.add(f"{step.name.value}: {step.status.name}")
