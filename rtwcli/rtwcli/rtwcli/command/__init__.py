...
def add_subparsers(
    parser, cli_name, dest, command_extensions, hide_extensions=None, required=True
):
    """
    ...
    warnings.warn(
        "'rtwcli.command.add_subparsers' is deprecated, use "
        "'rtwcli.command.add_subparsers_on_demand' instead",
        stacklevel=2,
    )
    ...
    if command_extensions:
        ...
        for name in sorted(command_extensions.keys()):
            ...
            description += f"{name.ljust(max_length)}  {get_first_line_doc(extension)}\n"
